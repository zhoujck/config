 ...
 * 关键修复：POST data必须传JSON对象，不能传form字符串
 */
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        return JSON.stringify({list: VODS, page: pg, pagecount: 9999, limit: 30, total: 999999});
    } catch (e) {
...
async function detail(id) {
    try {
        ...
        var t = Math.floor(Date.now() / 1000).toString();
        var detailKey = Encrypt(JSON.stringify({"token_id":"393668","vod_id":vod_id,"mobile_time":t,"token":TOKEN_STR}));
        var sigPrefix = 'token_id=,token=' + TOKEN_STR + ',phone_type=1,request_key=' + detailKey + ',app_id=1,time=' + t + ',keys=Qmxi5ciWXbQzkr7o+SUNiUuQxQEf8/AVyUWY4T/BGhcXBIUz4nOyHBGf9A4KbM0iKF3yp9M7WAY0rrs5PzdTAOB45plcS2zZ0wUibcXuGJ29VVGRWKGwE9zu2vLwhfgjTaaDpXo4rby+7GxXTktzJmxvneOUdYeHi+PZsThlvPI=*&zvdvdvddbfikkkumtmdwqppp?|4Y!s!2br';
        var signature = md5(sigPrefix);

        var body1 = {
            token: TOKEN_STR, token_id: '', phone_type: '1', time: t,
            phone_model: 'xiaomi-22021211rc',
        ...
        var playSigPrefix = 'token_id=,token=' + TOKEN_STR + ',phone_type=1,request_key=' + playKey + ',app_id=1,time=' + t + ',keys=Qmxi5ciWXbQzkr7o+SUNiUuQxQEf8/AVyUWY4T/BGhcXBIUz4nOyHBGf9A4KbM0iKF3yp9M7WAY0rrs5PzdTAOB45plcS2zZ0wUibcXuGJ29VVGRWKGwE9zu2vLwhfgjTaaDpXo4rby+7GxXTktzJmxvneOUdYeHi+PZsThlvPI=*&zvdvdvddbfikkkumtmdwqppp?|4Y!s!2br';
...
async function play(flag, id, flags) {
    try {
        ...
        return JSON.stringify({parse: 0, url: url, header: {'User-Agent': MOBILE_UA, 'Referer': HOST}});
    } catch (e) {
...
async function search(wd, quick, pg) {
    try {
        pg = parseInt(pg, 10) || 1;
        var body = buildSignedBody({"keywords": wd, "order_val": "1"});
        ...
        return JSON.stringify({list: VODS, page: pg, pagecount: 9999, limit: VODS.length, total: 999999});
    } catch (e) {
