...
const SEARCH_KEYS = "qDpotE2bedimK3QGqlyV5ieXXC3EhaPLQ+IOJyHnHflCj5w/7ESK7FgywMvrgjxbx0GklEFLI4+JshgySe633OIRstuktwdiCy3CT+fLSpuxBJDIlfXQDaeH3ig1wiB0JsZ601XHiFweGMu4tZfnSpHg3OnoL6nz/uurUif2OK4=";
...
async function home(filter) {
    try {
        ...
        var body = buildBodyUpper(requestKey, timestamp, SEARCH_KEYS);
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        var timestamp = Math.floor(Date.now() / 1000).toString();
        var requestKey = Encrypt(JSON.stringify({
            area: area, year: year, pageSize: '20', sort: sort, page: pg.toString(), tid: tid
        }));
        var body = buildBodyUpper(requestKey, timestamp, SEARCH_KEYS);
        var data = await apiPost('/App/IndexList/indexList', body);
        var vods = [];
